...
dnl
dnl COPYRIGHT (c) 2019 The SML/NJ Fellowship.
dnl
...
AC_DEFUN([CHECK_SMLNJ_HEAP_SUFFIX], [
  ...
  if test z$SMLNJ_ARCH != z -a z$SMLNJ_OPSYS != z ; then
    if test $SMLNJ_ARCH = x86_64 ; then
      # SMLNJ_ARCH is either x86 or amd64
      case x"$HOSTSIZE" in
	x32) SMLNJ_ARCH=x86 ;;
	x64) SMLNJ_ARCH=amd64 ;;
	*) SMLNJ_ARCH=x86 ;;	# default to 32-bits for now
      esac
    fi
